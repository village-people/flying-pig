...
def get_batch_schedule(partition, batch_size):
    import torch
    schedules = []
    for (scheduler_name, p, args) in partition:
        n = int(batch_size * p)
        if n > 0:
            schedules.append((ALL_SCHEDULES[scheduler_name](*args), n))

    epsilon = torch.zeros(batch_size)
    while True:
        start = 0
        for schedule, n in schedules:
            value = next(schedule)
            epsilon[start:(start+n)].fill_(next(schedule))
            start += n
        yield epsilon
